-- EraUI trade skill window.
-- A profession's own window as the old trade skill window: the rank bar under
-- the title, the recipe list in the upper half under headers that fold, each
-- recipe in the color of its difficulty, and the chosen recipe in the lower
-- half with its icon, what it requires and its reagents; Create All, a count,
-- Create and Exit along the foot.
local _, EraUI = ...
...
function ns.OldSkillShell(panel, opts)
    ...
    local function Piece(layer)
        ...
        ns.ThemeTexture(tex, "frameMetal")
        return tex
    end
...
local function Build()
    ...
    panel = CreateFrame("Frame", "EraUIClassicTradeSkill", host)
