-- EraUI action-button skinning.
-- The 1.x button art on Blizzard's action buttons: the recessed socket behind
-- every button and the bevelled ring over it, so they read as stone on the
-- band instead of black squares.
local _, EraUI = ...
...
-- With the classic bar turned off, Action Bar 1 is the game's own bar in its
-- own art, and the slot art (the wing behind each empty slot) is part of it.
...
-- The client repaints a button's art on its own passes; the art is watched,
-- and only a repainted button is done again.
