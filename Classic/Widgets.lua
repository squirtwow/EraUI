-- EraUI widget skinning.
-- The old controls put onto the client's modern widgets: the red panel
-- button, check box, slider, drop down, page steppers, the thin scroll bar's
-- knob and arrows, and the tabs. Each skinner takes the widget Blizzard made
-- and fades its art; the widget's behavior is untouched.
local _, EraUI = ...
...
-- The red button of the old panels on a modern three-slice or panel button.
...
-- A page arrow on a modern icon button (the steppers beside a drop down or
-- slider).
...
-- The old slider bar under a modern slider: the bordered track and the round
-- button; the client's end arrows are kept, drained of their bronze color.
function ns.SkinSliderWithSteppers(frame)
    ...
    -- The client re-applies its own (bronze) arrow art whenever it sets a
    -- slider up again, so the arrows are desaturated rather than hidden.
...
-- drawn on any frame (drop downs in the settings window, and any of ours).
...
-- The old knob on a modern thin scroll bar. Blizzard's thumb is a long piece
-- whose length follows the content, so the knob is placed on the track from
-- the scroll fraction instead (top of the track at 0, bottom at 1).
local KNOB_H = 24
function ns.ClassicKnob(bar)
    ...
    local function Place()
        ...
        -- The client's track stops three pixels short of its arrows at both
        -- ends; the old knob ran right up to them.
...
-- The old scroll column behind a thin scroll bar: a bordered track with a
-- socket for the arrow at each end, from the character sheet's scroll sheet.
...
function ns.ScrollTrackArt(bar)
    ...
    -- Fitted through a frame of ours laid over the bar, since a hook on the
    -- bar's own size script was not always called (the first time a window
    -- opened, the column was fitted to a stale height).
...
-- The same column behind one of the addon's own scroll bars, whose arrows
-- stand outside the bar, above and below it.
...
-- Every thin scroll bar inside a window, a few levels down (gossip and quest
-- text panes, mail, trade and other lists). A forbidden frame somewhere under
-- a window (the bank keeps one) may not even be asked for its children.
...
-- A modern minimal tab in the old tab pieces, turned over so its base sits on
-- the box below it; the active sheet while it is selected.
...
-- The old item name plate: its art sits inside a border of empty pixels, so a
-- texture drawn at the size you want comes out smaller. This takes the size
-- the plate should read at and sets the texture that gives it.
...
-- How bright the dark marble of a list pane is drawn.
ns.PANE_SHADE = 0.9
...
-- A panel that stands on its own over the world: the old menus were solid
-- black with a thin silver line around them.
...
-- The list an old drop down opened, hung under the drop down it belongs to.
...
-- The little menu the old lists opened under the cursor on a right click.
-- Entries are { text, onClick, allowed }.
...
-- A column header: the old who-list tab cut in three, one per column.
...
-- Escape closes a window of ours. The client reads UISpecialFrames in the
-- middle of its Escape handling, so an entry there makes that handling ours;
-- instead the key is taken while a window of ours is up, bound to a button
-- that closes the topmost, and handed back the moment none is. Bindings cannot
-- change in combat, so the key is handed back as a fight begins and taken
-- again after it if a window is still up.
local escButton
local escFrames = {}
...
local function EscOurs()
    ...
    return ok and type(action) == "string" and action:find("EraUIClassicEscButton", 1, true) ~= nil
end
...
local function EscUpdate()
    ...
    if want then
        SetOverrideBindingClick(escButton, true, "ESCAPE", "EraUIClassicEscButton")
    end
end
...
function ns.CloseOnEscape(frame, closer)
    ...
    if not escButton then
        escButton = CreateFrame("Button", "EraUIClassicEscButton", UIParent)
        escButton:SetSize(1, 1)
        escButton:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 0, 0)
        escButton:EnableMouse(false)
        escButton:Show()
        -- A key bound to a button clicks it on press or release depending on
        -- the game's cast-on-key-down setting, so listen for both.
