-- EraUI bar-button skinning.
-- Classic art on Blizzard's bag and micro buttons. Each skin is applied once,
-- undone on demand, and put back after Blizzard's own state changes through
...
-- Micro button -> classic art name. Buttons with no 1.x counterpart keep
-- their modern art.
local MICRO_ART = {
    ...
    EraUIClassicWorldMapMicroButton = "World",
}
...
local function HookMicro(button)
    ...
    -- Blizzard fades the normal texture out on mouseover because its highlight
    -- atlas is a whole button; the 1.x sheet stays put.
...
-- A micro button is down while its window is up, and up when it is not. Kept
-- by looking (a few times a second) rather than by asking the client to go
-- over its buttons, which knows nothing of our windows and may not be asked
-- in combat.
local MICRO_WINDOWS = {
    CharacterMicroButton = { "CharacterFrame" },
    ProfessionMicroButton = { "ProfessionsFrame", "ProfessionsBookFrame" },
    SpellbookMicroButton = { "EraUIClassicSpellBook" },
    TalentMicroButton = { "EraUIClassicTalents" },
    PlayerSpellsMicroButton = { "EraUIClassicTalents" },
    AchievementMicroButton = { "AchievementFrame" },
    QuestLogMicroButton = { "EraUIClassicQuestLog" },
    ...
    EraUIClassicWorldMapMicroButton = { "WorldMapFrame" },
}
...
-- without a key ring the reagent bag wears it. Blizzard refreshes bag
-- textures often; the hook puts it back.
local keyrings = {}
