-- EraUI bag windows.
-- The 1.x bag windows on the client's container frames: the old bag sheet
-- drawn in three pieces (a top with the portrait ring and name, one middle
-- piece per row, a bottom lip), the backpack's own sheet with the money
-- strip, the slots re-laid on the old 41 pixel row pitch, and the old slot
-- border under each icon.
local _, EraUI = ...
...
local ROW = 41
local COL = 42
local FIRST_X, FIRST_Y = -12, 9
local ROWS_PER_PIECE = 6
local BACKPACK_ROWS_PER_PIECE = 5
local ROW_BAR_TOP = 213 / 512
local SHEET_H = 512
local MIDDLE_TOP = 0.353515625
local FIRST_ROW_PIXELS = 9
local BOTTOM_TOP, BOTTOM_BOTTOM, BOTTOM_H = 0.330078125, 0.349609375, 10
local BACKPACK_ROWS = 4
local BACKPACK_TOP, BACKPACK_SPLIT = 255, 210
local BACKPACK_BASE_H = 240
local BACKPACK_FIRST_Y = -211
local BACKPACK_MIDDLE_TOP = 215 / 512
local MONEY_Y = -215

local active = false
local hooked = setmetatable({}, { __mode = "k" })

local STRIP_L, STRIP_R = 162, 204
...
local function Skin(frame)
    if not active or not frame or not frame.GetBagSize then return end
