-- EraUI quest tracker.
-- The old quest watch look on Blizzard's objective tracker: plain text, no
-- header plates or glows. Section titles small and gold, quest titles in the
-- level color with the level in front, objectives one size smaller in gray
-- that turns white as they progress and green when done.
local _, EraUI = ...
...
local function SkinMainHeader(header)
    if not header or not active then return end
    local collapsed = ObjectiveTrackerFrame.isCollapsed
    PlainHeader(header, collapsed, "headerOpen", "headerClosed")
    if collapsed then
...
local function Apply()
    ...
    if not hooked then
        ...
        if ObjectiveTrackerFrame.Header then
            ...
            ObjectiveTrackerFrame.Header:HookScript("OnShow", function(self) SkinMainHeader(self) end)
        end
        ns.HookMethod(ObjectiveTrackerFrame, "Update", function() if active then SkinMainHeader(ObjectiveTrackerFrame.Header); SkinModules() end end)
    end
end
...
local function Restore()
    ...
    if header then
        if ObjectiveTrackerFrame.ShouldShowHeader and ObjectiveTrackerFrame:ShouldShowHeader() then header:Show() end
        for _, key in ipairs({ "Background", "Shine", "Glow" }) do
