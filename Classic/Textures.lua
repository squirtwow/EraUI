-- EraUI texture registry.
-- Maps logical keys to the Classic art the game client still ships ("builtin")
-- and to the bundled Classic Era copies under Media/Classic/ ("bundled"), so a
-- missing client file never leaves a blank region. Paths are Blizzard's own art.
local _, EraUI = ...
local ns = EraUI.Classic

local BUNDLED = "Interface\\AddOns\\EraUI\\Media\\Classic\\"
...
ns.TEX = {
    ...
    -- Shaded only down its height; the experience-bar fill is tiled, so the
    -- classic sheet would render as a row of dark/light blocks. Both names
    -- point at the flat copy in Media/.
    ...
    -- The client's copy of this sheet is a wider redraw; the bundled one is
    -- the 128x32 original, used on purpose.
    ...
    -- quest log (the client ships these as near-black stand-ins; the bundled
    -- copies are the Classic Era originals).
    ...
    -- scrollbar (client ships modern stand-ins; bundled copies are Classic Era).
    ...
    -- reputation / skills plates (bundled Classic Era originals).
...
-- Two the client has redrawn under their old names: the Socials micro button
-- is a guild banner now, and the "no tracking" icon is no longer the glass.
...
for key, file in pairs({
    ...
    ns.TEX[key] = { builtin = "Interface\\" .. file, bundled = BUNDLED .. name }
end

-- Result of the last SetTexture per key, for the debug command.
ns.texStatus = {}
...
-- SetTexture returns false when the file does not exist; fall back to the other
-- copy so a missing client file never leaves a blank region.
function ns.SetTex(texture, key)
    ...
    if ns.ThemeTexture then ns.ThemeTexture(texture, key) end
    return ok ~= false
end
