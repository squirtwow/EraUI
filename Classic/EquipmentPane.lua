-- EraUI equipment pane.
-- The equipment manager beside the classic sheet. 1.x had no set manager;
-- Wrath put a small arrow on the paper doll that opened the sets in a dialog
-- docked to the right of the window. That is what this does: an arrow on the
-- sheet, and Blizzard's own equipment manager pane inside a dialog of ours.
local _, EraUI = ...
...
local function SetOpen(state)
    ...
    if EventRegistry and EventRegistry.TriggerEvent then EventRegistry:TriggerEvent("EraUIClassic.CharacterSheetLaid") end
end

local function Build()
    if pane or not PaperDollFrame or not CharacterFrame then return end
    pane = CreateFrame("Frame", "EraUIClassicEquipmentPane", PaperDollFrame, "BackdropTemplate")
    ...
    toggle = CreateFrame("Button", "EraUIClassicEquipmentToggle", PaperDollFrame)
