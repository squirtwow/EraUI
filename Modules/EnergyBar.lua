local _,EraUI=...
local Module={}
EraUI:RegisterModule("EnergyBar",Module)
local bar,control,edge,text,resizePanel
local editing=false
local ENERGY=3
local function Options()
    EraUIDB.energyBarPosition=EraUIDB.energyBarPosition or {x=0,y=-164,width=160,height=20}
    local p=EraUIDB.energyBarPosition
...
local function Enabled()
    ...
    return EraUI:GetSetting("enabled") and EraUI:GetSetting("energyBar") and class=="ROGUE"
end
...
local function Build()
    ...
    bar=CreateFrame("StatusBar","EraUIEnergyBar",UIParent)
    ...
    control=CreateFrame("Button","EraUIEnergyControl",bar,"BackdropTemplate")
    ...
    control:SetScript("OnEnter",function(self)
        Hover()
        if GameTooltip then
            GameTooltip:SetOwner(self,"ANCHOR_TOP");GameTooltip:SetText("Energy bar")
    ...
    resizePanel=CreateFrame("Frame","EraUIEnergyResizeControls",bar,"BackdropTemplate")
...
function Module:Refresh()
    ...
    bar:SetValue(UnitPower("player",ENERGY))
    text:SetFormattedText("%d",UnitPower("player",ENERGY))
    bar:Show();Hover()
end
function Module:Initialize()
    ...
    events:SetScript("OnEvent",function(_,event,unit,power)
        if (event=="UNIT_POWER_UPDATE" or event=="UNIT_POWER_FREQUENT") and power~="ENERGY" then return end
